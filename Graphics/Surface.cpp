...
Surface::Surface(Graphics& gfx, const std::wstring& fileName, bool sRGB)
    :mFilename(fileName)
{
    ...
    auto heapHandle = gfx.AllocHeap();
    mCpuHandle = heapHandle.first;
    mGpuHandle = heapHandle.second;
}
...
void Surface::Bind(Graphics& gfx)
{
    GraphicContext::GetCommandList(gfx)->SetGraphicsRootDescriptorTable(1, mGpuHandle);
}
...
void Surface::Upload(Graphics& gfx)
{
    ...
    d3d12Device->CreateShaderResourceView(mTexureBuffer.Get(), &srvDesc, mCpuHandle);
