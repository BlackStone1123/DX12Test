	...
	, phi( adist( rng ) )
{
	if (!IsStaticInitialized())
	{
		...
		auto tableNode = root->AddSubNode()->SetType(ParameterType::TABLE)->SetVisibility(D3D12_SHADER_VISIBILITY_PIXEL);
		tableNode->AddSubNode()->SetType(ParameterType::SRV)->SetNumDescriptor(2)->SetBaseRegister(0);
		tableNode->AddSubNode()->SetType(ParameterType::CBV)->SetNumDescriptor(1)->SetBaseRegister(0);
